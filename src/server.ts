...
export function createMcpServer(): McpServer {
    ...
    server.registerResource(
        ...
        {
            description:
                "Crime Evidence Management System demo configuration. Contains blockchain, storage, security, roles, evidence lifecycle settings, and any dynamically-added sections.",
    ...
    // -------------------------------------------------------------------------
    server.registerTool(
        "update_demo_resource",
        {
            description:
                "Update a specific value in the Crime Evidence system demo configuration. " +
            ...
            inputSchema: {
                section: z
                    .string()
                    .describe(
                        "Top-level section name in the config (e.g. 'security', 'blockchain', 'storage', 'database', 'roles', 'evidence_lifecycle', 'performance', 'notifications', 'system')"
                    ),
                key: z
                    .string()
                    .describe(
                        "Key within the section to update (e.g. 'session_timeout_min', 'mfa_enabled')"
                    ),
                value: z
                    .any()
                    .describe(
                        "New value. Can be string, number, boolean, object, or array."
                    ),
            },
        },
    ...
    server.registerTool(
        ...
        {
            description:
                "Add a new dynamic section to the Crime Evidence system configuration. " +
            ...
            inputSchema: {
                section_name: z
                    ...
                    .min(1)
                    .describe(
                        "Name for the new dynamic section (e.g. 'audit_settings', 'mobile_config')"
                    ),
                schema: z
                    .record(z.any())
                    .describe(
                        "Key-value object defining the section's initial configuration"
                    ),
                overwrite: z
                    .boolean()
                    .optional()
                    .default(false)
                    .describe(
                        "If true, overwrites an existing dynamic section with the same name. Default: false."
                    ),
            },
        },
